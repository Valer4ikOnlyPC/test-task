...
refresh-cron:
	$(docker_compose_bin) exec -T -u $(shell id -u) "$(PHP_CLI_CONTAINER_NAME)" sh -c "crontab -u root /etc/periodic/crontab"
	$(docker_compose_bin) exec -T -u $(shell id -u) "$(PHP_CLI_CONTAINER_NAME)" sh -c "service cron reload"

migrate:
	$(docker_compose_bin) exec -T -u $(shell id -u) "$(PHP_CLI_CONTAINER_NAME)" ./test-task/apps/main/bin/console doctrine:migrations:migrate

create-desadv-from-xml: up ## Execute create-desadv-from-xml with ARGS="<file-name>"
	$(docker_compose_bin) exec -u $(shell id -u) $(PHP_CLI_CONTAINER_NAME) ./test-task/apps/main/bin/console core:desadv:create-from-file $(ARGS)

generate-number:
	$(docker_compose_bin) exec -u $(shell id -u) $(PHP_CLI_CONTAINER_NAME) ./test-task/apps/main/bin/console core:number-generation:generate
